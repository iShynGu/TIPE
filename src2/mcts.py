...
class MCTSNode:
    ...
    def is_fully_expanded(self):
        return len(self.children) == len(coup_possible(self.state, self.state["pacman"]))

    def select_child(self):
        scores = [ucb_score(child, self.visit_count, 1.4) for child in self.children]
        return self.children[np.argmax(scores)]

    def expand(self):
        possible_moves = coup_possible(self.state, self.state["pacman"])
        move = random.choice(possible_moves)
        new_state = copy.deepcopy(self.state)
        deplace(new_state, "pacman", move[0], move[1])
...
def select_best_child(node):
    ...
    for child in node.children:
        score = child.reward / child.visit_count
        if score > max_score:
            max_score = score
            best_child = child
    return best_child
...
def expand(node):
    possible_moves = coup_possible(node.state, node.state["pacman"])
    for move in possible_moves:
...
def ucb_score(node, parent_visits, exploration_constant):
    if node.visit_count == 0:
        return float("inf")
    return (
        node.reward / node.visit_count
        + exploration_constant * np.sqrt(np.log(parent_visits) / node.visit_count)
    )


def uct_search(root, num_iterations, depth_limit):
    for _ in range(num_iterations):
        node = root
        state = copy.deepcopy(root.state)

        # Selection
        while not node.is_fully_expanded() and not fin_partie(state):
            if node.children:
                node = node.select_child()
                deplace(state, "pacman", node.state["pacman"][0], node.state["pacman"][1])
            else:
                node = node.expand()

        # Expansion
        while not fin_partie(state) and node.children:
            node = node.select_child()
            deplace(state, "pacman", node.state["pacman"][0], node.state["pacman"][1])

        # Rollout
        if not fin_partie(state):
            for _ in range(depth_limit):
                i, j = meilleur_coup_pac(state)
                k, l = astar_for_ghost(state, depth_limit)
                deplace(state, "pacman", i, j)
                state["f"] = [k, l]
                mange_fruit(state)
                if fin_partie(state):
    ...
    return best_child.state["pacman"]
...
def mcts_pacman():
    ...
    while not fin_partie(state) and not victoire(state):
        ...
        while not vie_en_moins(state):
            best_child = uct_search(root, 1000, 5)  # Adjust the number of iterations and depth limit as needed
            move = best_child.state["pacman"]
            deplace(state, "pacman", move[0], move[1])
            state["f"] = astar_for_ghost(state)
            mange_fruit(state)
            affiche(state)
            print()
            time.sleep(0.5)
        ...
        reset_map(state["nbr_vies"], state)
        print()
